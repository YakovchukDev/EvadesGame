            ...
            for (int row = 0; row < _map.GetLength(0); row++)
            {
                ...
                for (int column = 0; column < _map.GetLength(1); column++)
                {
                    ...
                    else
                    {
                        ...
                        if ((row > 0 && _map[row - 1, column] != null) || column - 1 == _branchs ||
                            column + 1 == _branchs)
                        {
                ...
                for (int row = 0; row < _map.GetLength(0); row++)
                {
                    if (_map[row, column] != null)
                    {
                        ...
                        _map[row, column] = Instantiate(_map[row, column],
                            new Vector3(
                                _arena.GetLengthX() / 2f + _saveZone.GetLengthX() / 2f + _arena.GetLengthX() * row +
                                _saveZone.GetLengthX() * row, 0f, _arena.GetLengthZ() * column), Quaternion.identity);
                    }
                }
                ...
                for (int row = 0; row < _saveZoneMap.GetLength(0); row++)
                {
                    if (_saveZoneMap[row, column] != null)
                    {
                        ...
                        _saveZoneMap[row, column] = Instantiate(_saveZoneMap[row, column],
                            new Vector3(_arena.GetLengthX() * row + _saveZone.GetLengthX() * row, 0,
                                _saveZone.GetLengthZ() * column), Quaternion.identity);
                    }
                }
            }
        }
